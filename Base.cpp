...
unsigned char Base::handle_io(unsigned char* inst_ptr,unsigned int* xax, unsigned int xdx){
    ...
    switch(inst_ptr[inst_indx]){
        ...
        case OPCODE_OUT_IMM8_AL:
            ...
            output_handler->process(current_port,xax);
            return 2 + inst_indx;        
        ...
        case OPCODE_OUT_DX_AL:
            output_handler->process(xdx, xax);
            return 1;
        ...
        case OPCODE_IN_AL_IMM8:
            current_port = inst_ptr[inst_indx+1];        
            input_handler->process(current_port, xax);
            return 2 + inst_indx;
